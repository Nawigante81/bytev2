...
function ProductDetailPage() {
    ...
    if (product && selectedVariant) {
      ...
      try {
        ...
        toast({
          ...
          description: `${quantity} x ${product.title} (${selectedVariant.title}) dodano.`,
        });
      } catch (error) {
        toast({
          variant: "destructive",
    ...
    return (
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center text-red-400 p-8 glass-card rounded-2xl">
          ...
          <p className="mb-6">Błąd ładowania produktu: {error}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <>
      <Helmet>
        <title>{product.title} - Sklep ByteClinic</title>
        ...
        <div className="grid md:grid-cols-2 gap-8 glass-card p-8 rounded-2xl">
              ...
              <div className="flex justify-center gap-2 mt-4">
                {product.images.map((_, index) => (
                  <button
                    ...
                    aria-label={`Idź do zdjęcia ${index + 1}`}
                  />
                ))}
              </div>
            )}
          ...
          <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.5, delay: 0.2 }} className="flex flex-col">
            ...
            {product.variants.length > 1 && (
              <div className="mb-6">
                <h3 className="text-sm font-medium text-white mb-2">Wariant</h3>
                <div className="flex flex-wrap gap-2">
                  {product.variants.map(variant => (
                    <Button
                      ...
                      className={`transition-all ${selectedVariant?.id === variant.id ? 'bg-primary border-primary' : 'border-white/20 text-white hover:bg-white/10'}`}
                    >
                      {variant.title}
                    </Button>
                  ))}
            ...
            <div className="mt-auto">
              <Button onClick={handleAddToCart} size="lg" className="w-full bg-gradient-to-r from-primary to-secondary hover:from-primary/80 hover:to-secondary/80 text-white font-semibold py-3 text-lg disabled:opacity-50 disabled:cursor-not-allowed" disabled={!canAddToCart || !product.purchasable}>
