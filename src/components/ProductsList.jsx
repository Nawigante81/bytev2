  ...
  const handleAddToCart = useCallback(async (e) => {
    ...
    try {
      ...
      toast({
        ...
        description: `${product.title} został dodany do Twojego koszyka.`,
      });
    } catch (error) {
      toast({
        title: "Błąd dodawania do koszyka",
  ...
  return (
    ...
    >
      <Link to={`/product/${product.id}`}>
        <div className="rounded-xl border border-white/10 bg-card/80 text-card-foreground shadow-sm backdrop-blur-sm overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:-translate-y-1 hover:shadow-primary/20">
          <div className="relative">
            ...
            <div className="absolute top-3 right-3 bg-primary/80 text-white text-xs font-bold px-3 py-1 rounded-full flex items-baseline gap-1.5">
              {hasSale && (
          ...
          <div className="p-4">
            ...
            <p className="text-sm text-gray-300 h-10 overflow-hidden">{product.subtitle || 'Sprawdź ten niesamowity produkt!'}</p>
  ...
  if (error) {
    return (
      <div className="text-center text-red-400 p-8">
        <p>Błąd ładowania produktów: {error}</p>
      </div>
    );
  }
  ...
  if (products.length === 0) {
    return (
      <div className="text-center text-gray-400 p-8">
        <p>Brak dostępnych produktów w tym momencie.</p>
      </div>
    );
  }
