...
const ECOMMERCE_STORE_ID = "store_01K6Y4B51RJTPPAM8A032R7GYV";
...
export class ApiError extends Error {
	...
	getUserMessage() {
		switch (this.status) {
			case 400:
				return 'Nieprawidłowe dane żądania. Sprawdź wprowadzone informacje.';
			case 401:
			...
			default:
				return this.message || 'Wystąpił nieoczekiwany błąd.';
		}
	}
}
		...
		try {
			...
			if (!response.ok) {
				...
				if (config.retryableStatuses.includes(response.status) && attempt < config.maxRetries) {
					...
					console.warn(`[API] Retry ${attempt + 1}/${config.maxRetries} for ${url} after ${Math.round(delay)}ms`);
					await sleep(delay);
					lastError = error;
					continue;
				}
			...
			if (error.name === 'TypeError' || error.name === 'AbortError') {
				...
				if (attempt < config.maxRetries) {
					const delay = calculateDelay(attempt, config);
					console.warn(`[API] Network retry ${attempt + 1}/${config.maxRetries} for ${url} after ${Math.round(delay)}ms`);
					await sleep(delay);
...
export async function initializeCheckout({items, successUrl, cancelUrl, locale}) {
	...
	const response = await fetchWithRetry(url, {
		...
		maxRetries: 2, // Fewer retries for checkout to avoid duplicate orders
	});
