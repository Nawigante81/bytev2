...
[${timestamp}] ${message}${colors.reset}`);
  }
...
json();
    ...
    try {
      ...
      if (spfRecord) {
        ...
        if (spfRecord.includes('include:_spf.resend.com')) {
          ...
          this.log('  ⚠ SPF nie zawiera Resend (_spf.resend.com)',
 ...
 'warning');
    ...
    try {
      ...
      this.log(`✗ Błąd sprawdzania DKIM: ${error.
...
xRecords.forEach(mx => {
  ...
  async testCORS() {
    ...
    try {
      ...
      const corsHeaders = {
        'access-control-allow-origin': response.headers.get('access
...
this.results.warnings.push(`Env: Brak ${name}`);
  ...
  generateReport() {
    ...
    this.log(`Ostrzeżenia: ${this.results.warnings}`, 'warning');
